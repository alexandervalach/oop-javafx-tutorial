...
public class TracksController extends BaseController
{
  ...
  /**
   * Shows detailed information about selected track
   * @param track displayed track
   */
  ...
  /**
   * Procedure to register event listeners
   */
  ...
  private void showAddDialog() throws IOException {
    ...
    loader.setLocation(FileHelper.getLocation(viewURL, "tracks/add"));
  ...
  /***
   * Synchronizes list of tracks with serialized file
   * @throws IOException if input file is not present
   * @throws ClassNotFoundException if Track does not exist
   */
  private void syncTracks () throws IOException, ClassNotFoundException {
    String dataURL = rootURL + "/data/tracks.txt";

    if (FileHelper.fileExists(dataURL)) {
